    ...
    try:
        ...
        try:
            ...
            if playwright_available:
                exported_count = _export_charts_with_playwright(models, leaderboard_data, l1_dims, imgs_dir, timestamp, logger)
            else:
                ...
                if chart_lib_available:
                    exported_count = _export_charts_with_matplotlib(models, leaderboard_data, l1_dims, imgs_dir, timestamp, logger)
                else:
                    exported_count = _export_charts_placeholder(models, imgs_dir, timestamp)
        
        except Exception as chart_export_error:
            logger.error(f"Error in chart export logic: {chart_export_error}", exc_info=True)
            exported_count = _export_charts_placeholder(models, imgs_dir, timestamp)
        
        flash(f'成功导出了 {exported_count} 个图表到 ./temp/imgs/ 文件夹。', 'success')
        ...
        flash('导出图表时发生错误，请检查日志。', 'danger')
    
    return redirect(url_for('index.index'))


def _export_charts_with_playwright(models, leaderboard_data, l1_dims, imgs_dir, timestamp, logger):
    """使用Playwright导出真实的图表"""
    from playwright.sync_api import sync_playwright
...
def _export_charts_with_matplotlib(models, leaderboard_data, l1_dims, imgs_dir, timestamp, logger):
    """使用matplotlib生成真实的图表"""
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
...
def _export_charts_placeholder(models, imgs_dir, timestamp):
    """创建占位符文件"""
    exported_count = 0
